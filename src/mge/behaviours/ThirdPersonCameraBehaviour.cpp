...
{
	glm::vec3 forward = glm::normalize(_owner->getLocalPosition() - _player->getActualWorldPosition());
...
void ThirdPersonCameraBehaviour::startTransition(glm::vec3 targetPos, float time)
{
	...
	_distanceProjectionLength = sqrt(_distance*_distance - _height*_height);
	_distanceVector = glm::vec3(_player->getWorldPosition().x, _height, _player->getActualWorldPosition().z + _distanceProjectionLength) - _player->getActualWorldPosition();
